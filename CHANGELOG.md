...
- A scan cited as evidence bounds what was scanned (e.g. surface-scan covers
  leak markers, not copied bytes); absence-of-bytes evidence is never proof
  of the negative.
...
- **What:** adds `choreography/codebase-map.md` (versioned snapshot contract defining Node/Leaf grammar, deterministic ordering, measure units, exclusion rules, and provenance fields), `build/check-codebase-map.py` (stdlib-only structural validator with 29-case self-test), `examples/codebase-map.valid.json`, and `examples/codebase-map.invalid.json`.
...
- **Evidence:** [VERIFIED — public conceptual source] the treemap concept is inspired by Yoann Padioleau's Codemap (GitHub: aryx/codemap; license: LGPL-2.1-only WITH OCaml-LGPL-linking-exception). Originality of the schema, validator, and fixtures is asserted by provenance review (the Provenance blocks in `choreography/codebase-map.md` and `demo/codebase-map/README.md`), not proven by scan: `python3 build/surface-scan.py` PASS is evidence of the absence of leak markers across its 8 surfaces, not proof that no line was informed by a source — a negative inspection cannot prove. [VERIFIED — internal operating record] validator self-test: 29/29 pass; surface-scan: PASS.
...
- **Evidence:** [VERIFIED — public conceptual source] the treemap concept is inspired by Yoann Padioleau's Codemap (GitHub: aryx/codemap; license: LGPL-2.1-only WITH OCaml-LGPL-linking-exception). Originality of the schema, validator, and fixtures is asserted by provenance review (the Provenance blocks in `choreography/codebase-map.md` and `demo/codebase-map/README.md`), not proven by scan: `python3 build/surface-scan.py` PASS is evidence of the absence of leak markers across its 8 surfaces, not proof that no line was informed by a source — a negative inspection cannot prove. [VERIFIED — internal operating record] browser read-back receipt recorded; console: no uncaught errors; network inspection: zero external requests; determinism verified (same fixture renders identical geometry twice).
