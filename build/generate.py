...
Contract (spec section 4):
  ...
  - Authored-fresh public files (AUTHORED_FRESH below) are copied verbatim —
    they have no manifest row by design and are never placeholder-substituted.
...
# ---------------------------------------------------------------------------
# Authored-fresh surfaces (the repository's authored-fresh mechanism).
#
# The kit's own surfaces — choreography/, build/ tooling, LICENSE, kit.yaml,
# README — are authored fresh: hand-written, with NO manifest row by design,
# and out of scope for the extraction gates (see sweep-gate.py). This generator
# ships the declared authored-fresh public files VERBATIM into the output, so a
# built kit carries its operating contracts and their checker alongside the
# extracted templates. Authored-fresh files are never extraction rows and are
...
def main():
    ...
    # ---- Authored-fresh public files ------------------------------------
    # Copied verbatim through dest_path() (the only destination-path
    # constructor), so no authored-fresh output can carry a raw instance-token
    # name. They count toward the shipped total: the AUDIT.md claim must close
    # against every file actually on disk.
    authored_files = 0
    for rel in AUTHORED_FRESH:
        src_root = os.path.join(ROOT, rel)
        if os.path.isdir(src_root):
            ...
            provenance.append(f"# AUTHORED-FRESH SKIP (no source): {rel}")
            continue
        for src_rel in sorted(paths):
            src = os.path.join(ROOT, src_rel)
            dst = dest_path(out, src_rel)
            os.makedirs(os.path.dirname(dst), exist_ok=True)
            shutil.copyfile(src, dst)
            shipped += 1
            authored_files += 1
    provenance.append(
        f"# AUTHORED-FRESH: {authored_files} file(s) copied verbatim "
        f"({', '.join(AUTHORED_FRESH)}) — no manifest row, no substitution")
    ...
    with open(os.path.join(out, "AUDIT.md"), "w", encoding="utf-8") as fh:
        ...
        fh.write(f"- of which authored-fresh: {authored_files} "
                 "(copied verbatim, no manifest row)\n")
