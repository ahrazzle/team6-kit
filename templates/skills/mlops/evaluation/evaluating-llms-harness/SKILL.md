<!-- GENERICIZED: 2×{AMOUNT}, 2×{CLIENT}, 13×{MODEL} | source: skills/mlops/evaluation/evaluating-llms-harness/SKILL.md -->
---
...
for model in models:
    with open(f"results/{model}.json") as f:
        ...
        for task in tasks:
            ...
            if "acc" in metrics:
                row[task.upper()] = f"{metrics['acc']:.{CLIENT}}"
            elif "exact_match" in metrics:
                row[task.upper()] = f"{metrics['exact_match']:.{CLIENT}}"
        results.append(row)
...
|------------------------|-------|-------|-----------|------------|
| {MODEL}  | 0.459 | 0.142 | 0.765     | 0.391      |
| {MODEL} | 0.549 | 0.287 | 0.801     | 0.430      |
